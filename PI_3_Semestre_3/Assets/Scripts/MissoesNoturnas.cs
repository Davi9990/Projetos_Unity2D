...
public class MissoesNoturnas : MonoBehaviour
{
    ...
    //Controle dos pontos ddo mapeamento inicial
    ...
    void Interagir()
    {
        ...
        Debug.Log("Interagir() foi chamado");
            ...
            case "PontoMapa":
                if(etapa == 0)
                {
                    ...
                    if(pontoAtual != null && !pontosVisitados.Contains(pontoAtual.transform))
                    {
                        ...
                        acaoCorreta = true;
                        Debug.Log($"Ponto de mapa observado({progressoAtual}/3)");

                        //Atualiza seta para o próximo ponto
                        indicePontoAtual++;
                        ...
                        //Se concluiu os 3 pontos, avança de etapa
                        if(progressoAtual >= 3)
                        {
                            IncrementarProgresso("2° Missão\nColete 4 ferramenta no armazem.", 1, 3, 4);
                    ...
                    {
                        Debug.Log("Ponto já visitado ou não encontrado.");
                    }
                }
                break;
    ...
    void AtualizarSeta()
    {
        ...
        //Etapa inicial (2 pontos de mapeamento)
        if(etapa == 0)
        {
            if(indicePontoAtual < locaisMissoes.Length)
            {
