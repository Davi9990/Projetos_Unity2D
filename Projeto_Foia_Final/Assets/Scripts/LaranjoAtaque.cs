...
{
    private GameObject jogador; // Referência ao GameObject do jogador
    ...
    public AudioClip efeitoPreRolagem;
    public AudioClip efeitoRolagem;

    void Start()
    {
        audioSource = GetComponent<AudioSource>();
        // Busca o jogador pela tag "Jogador"
        jogador = GameObject.FindGameObjectWithTag("Jogador");
        if (jogador == null)
        {
            Debug.LogError("Jogador não encontrado! Certifique-se de que o jogador tem a tag 'Jogador'.");
        }
    ...
    IEnumerator IniciarRolagem()
    {
        ...
        audioSource.PlayOneShot(efeitoPreRolagem, 0.1f);
        animador.SetTrigger("IniciarRolagemLaranjo");
        yield return new WaitForSeconds(0.5f); // Ajuste o tempo conforme a duração da animação IniciarRolagemLaranjo
        animador.SetBool("Rolando", true);
        audioSource.PlayOneShot(efeitoRolagem, 0.2f);
