...
public class QuizAntirracismo : MonoBehaviour
{
    ...
    public Text textoInteracaoQuiz;
    ...
    void Start()
    {
        ...
        botaoInteragir.onClick.AddListener(AbrirQuiz);
    }
    ...
    void FimDoQuiz()
    {
        ...
        //  Calcula a porcentagem de acertos como professor Danilo ajudou
        float percentualAcertos = ((float)pontos / perguntas.Count) * 100f;
        int percentualArredondado = Mathf.RoundToInt(percentualAcertos);

        // Exibe no painel quantos % o jogador acertou
        textoMensagem.text = 
            "Você acertou " + percentualArredondado + "% das perguntas.\n" +
            "Um dia, a liberdade não terá cor, e esse dia nascerá da nossa luta.";

        Debug.Log("Percentual de acertos: " + percentualArredondado + "%");

        //  Se o jogador acertou 70% ou mais, muda para a próxima fase se nao ele tem que repetir ate aprender
    ...
    IEnumerator IrParaFase2()
    {
        ...
        SceneManager.LoadScene("Cutscene 2"); // nao alterem, pois tem uma cutscene que eu coloquei para depois de passar de fase, ir para cutscene
    }
